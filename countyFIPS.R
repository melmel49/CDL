...
countyFIPS <- function(x,y){
  ...
  #find the index matched both county and state (I'm still thinking about this part, maybe grep() will be better)
    ids <- which((UScensusFIPS$STATE == x)|(toupper(UScensusFIPS$STATENAME) == x))
    id <- ids[ids %in% grep(y,toupper(UScensusFIPS$COUNTYNAME))]
